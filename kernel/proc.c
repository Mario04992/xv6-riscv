...
// Wait for a child and also return its CPU usage.
int
    ...
    for (pp = proc; pp < &proc[NPROC]; pp++) {
      if (pp->parent == p) {
        ...
        if (pp->state == ZOMBIE) {
          ...
          // Copy usage before reclaiming the child. Failed copyout
          // leaves the zombie available for a later wait attempt.
          struct rusage usage = { .cputime = pp->cputime };
          if (usageaddr != 0 &&
              copyout(p->pagetable, p->sz, usageaddr, (char *)&usage,
                      sizeof(usage)) < 0) {
            release(&pp->lock);
            release(&wait_lock);
            return -1;
